...
fn create_server_module(
    ...
    // Export names: individual tools + server name (for `import { server } from "server"`)
    ...
    Module::synthetic(
        ...
        SyntheticModuleInitializer::from_copy_closure_with_captures(
            |module, (tools, server_name), context| {
                // Create an object to hold all tools (for server-named export)
                ...
                for (tool_name, full_name) in tools.iter() {
                    ...
                    // Export as named export (e.g., `import { get_me } from "github"`)
                    module.set_export(&js_string!(tool_name.as_str()), js_func.clone().into())?;

                    // Also add to namespace object
                    namespace_obj
                ...
                // Export namespace object with server name
                // (allows `import { github } from "github"` to work)
...
fn wrap_code_for_result(code: &str) -> String {
    ...
    if is_continuation_line(last) {
        if let Some((decl_idx, decl)) = lines.iter().enumerate().rev().find(|(_, l)| {
            let trimmed = l.trim();
            DECLS.iter().any(|d| trimmed.starts_with(d)) && !trimmed.ends_with(';')
        }) {
            let decl_trimmed = decl.trim();
            let opens_literal = decl_trimmed.ends_with('{') || decl_trimmed.ends_with('[');
            let closes_literal =
                last == "};" || last == "];" || last == "}" || last == "]" || last == "});";
            let all_continuations = lines[decl_idx + 1..last_idx].iter().all(|l| {
                let t = l.trim();
                t.is_empty() || t.starts_with("//") || is_continuation_line(t)
            });
            if all_continuations || (opens_literal && closes_literal) {
                for d in DECLS {
                    if let Some(rest) = decl_trimmed.strip_prefix(d) {
...
mod tests {
    ...
    #[test_case(
        ...
        "__result__ = arr\n.map(x => x * 2)",
        "__result__ = arr;";
        "declaration without semicolon followed by var ref and chain captures chain"
    )]
    #[test_case(
        "const obj = {\n  x: 1,\n  y: 2\n};",
        "__result__ = obj;",
        "__result__ = x:";
        "multiline object literal declaration captures variable not interior"
    )]
    #[test_case(
        "const arr = [\n  1,\n  2,\n  3\n];",
        "__result__ = arr;",
        "__result__ = 1";
        "multiline array literal declaration captures variable not interior"
    )]
    #[test_case(
        "const arr: number[] = [\n  1,\n  2,\n  3\n];",
        "__result__ = arr;",
        "__result__ = arr:";
        "typescript type annotation extracts only identifier"
    )]
    ...
    fn test_namespace_import_with_synthetic_module() {
        // Test that all import styles work with synthetic modules
        ...
        // Test named import: import { tool } from "server"
        ...
        // Test namespace import: import * as server from "server"
        let code_namespace = r#"import * as testserver from "testserver"; typeof testserver.get_value"#;
        ...
        // Test server-named import: import { server } from "server"
        // This is what models often generate by mistake, now it works!
        let code_server_named = r#"import { testserver } from "testserver"; typeof testserver.get_value"#;
        ...
        // Test bracket notation with server-named import (for hyphenated tool names)
        let code_bracket = r#"import { testserver } from "testserver"; typeof testserver["get_value"]"#;
